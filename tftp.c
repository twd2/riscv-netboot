...
int get_file(char *filename, size_t filename_size, void *buff, size_t len, size_t *out_len)
{
...
__attribute__ ((aligned (16))) uint8_t dtb_buff[4096];

__attribute__ ((aligned (16))) uint8_t tftp_stack[1024];
static struct task tftp_task;
static void tftp_entry()
{
    ...
    void *dtb = 0;
    if (get_file("dtb", 4, dtb_buff, sizeof(dtb_buff), 0) == 0)
    {
        dtb = dtb_buff;
    }
    else
    {
        printf("Using builtin device tree.\r\n");
        // TODO: dtb = ???;
    }

    void *elf = (void *)(DRAM_BASE + DRAM_SIZE / 2);
    if (get_file(TFTP_FILENAME, TFTP_FILENAME_SIZE, elf, -1, 0) < 0)
    {
        exit();
    }

    void *kernel_entry = load_elf(elf);
    if (!kernel_entry)
    {
        exit();
    }
    ...
    reset_eth();
    enter_kernel(dtb, kernel_entry);
}

void init_tftp()
{
    create_task(&tftp_task, (uintptr_t)tftp_stack + sizeof(tftp_stack) + STACK_OFFSET, tftp_entry);
}
